...
if crm_cust is not None and erp_cust is not None and erp_loc is not None:
    try:
        ...
        # cst_id is already a clean nullable int from silver_transform.py - no re-cast needed here.
        crm_cust = crm_cust.sort_values("cst_key", ascending=False)
        crm_cust = crm_cust.drop_duplicates(subset="cst_id", keep="first")
        # ERP demographic/location sources key off string ids - build a throwaway string
        # key just for these merges so cst_id itself stays a clean int all the way through
        # (matches fact_sales.sls_cust_id, which Power BI needs for the relationship).
        crm_cust["_merge_key"] = crm_cust["cst_id"].astype(str).str.strip()
        ...
        dim_customers = crm_cust.merge(erp_cust, left_on="_merge_key", right_on="cid", how="left", suffixes=("", "_erp"))
        ...
        # Merge 2: + location
        dim_customers = dim_customers.merge(erp_loc, left_on="_merge_key", right_on=loc_key, how="left", suffixes=("", "_loc"))
        ...
        dim_customers.drop(columns=["_merge_key"], inplace=True)
...
if sales is not None:
    try:
        ...
        # Match sls_order_dt and any _dt suffix
...
if sales is not None:
    try:
        ...
        # sales_amount is computed once in silver_transform.py - read it as-is, don't recompute here
        if "sales_amount" in fact_sales.columns:
            print(f"      Using sales_amount from silver (computed once, not recalculated here)")
...
# Validation - orphan fact rows with no matching dimension row (non-blocking)
if all_success:
    print(f"\n[Validation] Checking fact_sales / dimension referential integrity...")
    try:
        cust_ids = set(dim_customers["cst_id"].dropna())
        prd_keys = set(dim_products["prd_key"].dropna())
        orphan_cust = (~fact_sales["sls_cust_id"].isin(cust_ids)).sum()
        orphan_prd  = (~fact_sales["sls_prd_key"].isin(prd_keys)).sum()
        print(f"    fact_sales rows: {len(fact_sales):,}")
        print(f"    Orphan rows (no matching dim_customers.cst_id): {orphan_cust:,}")
        print(f"    Orphan rows (no matching dim_products.prd_key): {orphan_prd:,}")
    except Exception as e:
