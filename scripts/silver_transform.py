...
if df is not None:
    try:
        ...
        # Fix numeric columns
        for col in ["sales_quantity", "unit_price", "unit_cost", "sales_price"]:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)

        # Derived column: sales amount
        if "sales_quantity" in df.columns and "unit_price" in df.columns:
            df["sales_amount"] = df["sales_quantity"] * df["unit_price"]
        ...
        if key_cols:
            ...
            if dropped > 0:
                print(f"      ⚠️  Dropped {dropped} rows with null key fields.")
        ...
        for col in ["bdate", "birthdate", "birth_date"]:
            if col in df.columns:
                ...
                if future_mask.sum() > 0:
                    print(f"      ⚠️  {future_mask.sum()} future birthdates found — setting to null.")
